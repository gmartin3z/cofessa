...
@section('title')
  Editar enlace (documentos y formatos)
@stop
    ...
    <div class="container">
      <div class="row">
        ...
        <div class="span12 aligncenter">
          ...
          <form action="{{ route('inicio.administrar.actualizar_enlace_documentos', [$submenu_documento->menu_id, $submenu_documento->submenu_id]) }}" method="post" id="frm_submenu">
            <input name="_method" type="hidden" value="put">
            {{ csrf_field() }}
            <div class="row controls">
              <div class="span6 control-group">
                <label>Descripción (requerido)</label>
                <input type="text" name="detalle_descripcion" value="{{ $submenu_documento->submenu_descripcion or old('detalle_descripcion') }}" maxlength="40" class="span6">
              </div>
              <div class="span6 control-group">
                <label>URL (requerido)</label>
                <input type="text" name="detalle_url" value="{{ $submenu_documento->url or old('detalle_url') }}" maxlength="255" class="span6">
              </div>
            </div>
            <div class="btn-toolbar">
              ...
              <br><br>
              <a class="btn btn-default" href="{{ route('inicio.administrar.seccion_enlaces_documentos', $submenu_documento->menu_id) }}" title="Ir al listado">
                <i class="icon-arrow-left"></i> Ir al listado
              </a>
            </div>
          </form>
          <form action="{{ route('inicio.administrar.borrar_enlace_documentos', [$submenu_documento->menu_id, $submenu_documento->submenu_id]) }}" method="post" id="frm_borrar_submenu">
            {{ csrf_field() }}
